...
Example:  from functools import reduce
	  ...
	  def cumulative_sum_of_squares_of_even_numbers(list_of_lists):
    		return [
        	reduce(lambda x, y: x + y,  # Reduce to get the sum
               	map(lambda num: num ** 2,  # Map to square each number
                filter(lambda num: num % 2 == 0, sublist)))  # Filter to select even numbers
...
Question: Rewrite the following program in one line by using nested filter, map and reduce
	  functions:
...
Example:  from functools import reduce
	  ...
	  def sum_squared_even(nums):
    		return reduce(lambda x, y: x + y,  # Reduce to get the sum
                       map(lambda x: x**2,  # Map to square each number
                       filter(lambda x: x % 2 == 0, nums)))  # Filter to select even numbers
...
Example:  list_of_lists = [['racecar', 'hello', 'madam'], ['python', 'level', 'stats'], ['abc', 'deed', 'xyz']]
...
Answer:   The generate_values function is not explicitly called to completion. Instead, a list comprehension 	 	  
	  ...
	  values until they are actually used in the list comprehension.
