...
def sum_squared_even(nums):
    return reduce(lambda x, y: x + y,  # Reduce to get the sum map(lambda x: x**2,  # Map to square each number
	   filter(lambda x: x % 2 == 0, nums)))  # Filter to select even numbers
...
#	  In the provided program, lazy evaluation is demonstrated by delaying the generation and processing of 	  	  
